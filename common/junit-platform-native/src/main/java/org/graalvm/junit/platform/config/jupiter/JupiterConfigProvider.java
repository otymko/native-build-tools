...
public class JupiterConfigProvider implements PluginConfigProvider {
    ...
    public void onTestClassRegistered(Class<?> testClass, NativeImageConfiguration registry) {
        ...
        AnnotationUtils.registerClassesFromAnnotationForReflection(testClass, registry, DisplayNameGeneration.class, source -> new Class[]{source.value()});
        AnnotationUtils.registerClassesFromAnnotationForReflection(testClass, registry, IndicativeSentencesGeneration.class, source -> new Class[]{source.generator()});
        ...
        AnnotationUtils.registerClassesFromAnnotationForReflection(testClass, registry, MethodSource.class, source -> JupiterConfigProvider.handleMethodReference(source.value()));
        AnnotationUtils.registerClassesFromAnnotationForReflection(testClass, registry, EnabledIf.class, source -> JupiterConfigProvider.handleMethodReference(source.value()));
        AnnotationUtils.registerClassesFromAnnotationForReflection(testClass, registry, DisabledIf.class, source -> JupiterConfigProvider.handleMethodReference(source.value()));
    }
